...
if not BOT_TOKEN:
    raise SystemExit("Environment variable BOT_TOKEN is not set. Set BOT_TOKEN and restart the bot.")
...
# Reduce noisy logs from third-party libs so console stays clean and no unexpected
# informational messages (appearing like ads) are shown.
for _lg in ("httpx", "telegram", "apscheduler", "urllib3", "googleapiclient", "gspread"):
...
def main():
    ...
    if app.job_queue is not None:
        ...
        logging.warning('JobQueue not installed. Install with: pip install "python-telegram-bot[job-queue]"')
